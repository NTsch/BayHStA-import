...
order by $collection//cei:titleStmt/cei:title
return <result>{concat($collection//cei:titleStmt/cei:title, ': ', substring-after($collection/base-uri(), 'cei/'))}</result>
...
(:return count($collections//cei:text[@type = 'charter'])
:)
